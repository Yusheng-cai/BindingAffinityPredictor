...
predicts affinity without coordinate diffusion or an MSA; and FlashBind scores
a FABind+-docked ligand in a three-dimensional receptor
...
compared predicted scores with experimental potency within each target. The
weighted Pearson correlations below closely reproduce the paper values. I
also evaluated Boltz-2 and the released FABind+ poses against 16 cocrystals;
those structure-recovery results are reported separately from affinity ranking.
...
\begin{tabular}{@{}lcc@{}}
\toprule
 & \textbf{Boltz-2} & \textbf{FlashBind/FABind+} \\
\midrule
...
\bottomrule
\end{tabular}\\[0.3em]
\footnotesize $^{*}$Supplied receptor coordinates, not a FlashBind protein prediction.
}}
\end{center}
