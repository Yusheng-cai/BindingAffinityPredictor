...
cd "$repo_root"
program_root="${BAP_PROGRAM_ROOT:-/home/yusheng/programs}"
cache_root="${BAP_CACHE_ROOT:-$repo_root/cache}"
nesso_env="$program_root/nesso-1.0.0"
boltz_env="$program_root/boltz-2.2.1"
...
if [[ "$model" == "nesso1" || "$model" == "all" ]]; then
  nesso_run="$run_root/nesso1/seed42/full50"
  env -u PYTHONPATH NESSO_CACHE="$cache_root/nesso" \
    "$nesso_env/bin/python" "$runner" \
    ...
    env -u PYTHONPATH NESSO_CACHE="$cache_root/nesso" \
    "$nesso_env/bin/nesso" predict \
    experiments/exp015_boltz2_nesso1_rnp_postcutoff50/inputs/nesso1/all \
    --out_dir "$nesso_run/raw" --cache "$cache_root/nesso" \
...
if [[ "$model" == "boltz2" || "$model" == "all" ]]; then
  boltz_run="$run_root/boltz2_msa1024/seed42/full50"
  env -u PYTHONPATH BOLTZ_CACHE="$cache_root/boltz" \
    "$boltz_env/bin/python" "$runner" \
    ...
    env -u PYTHONPATH BOLTZ_CACHE="$cache_root/boltz" \
    "$boltz_env/bin/boltz" predict \
    experiments/exp015_boltz2_nesso1_rnp_postcutoff50/inputs/boltz2/all \
    --out_dir "$boltz_run/raw" --cache "$cache_root/boltz" --model boltz2 \
    --checkpoint "$boltz_env/checkpoints/boltz2_conf.ckpt" \
